    ...
    export default {
    	data(){
    		return {
    			...
    			showAddCategory: false,//添加食品种类
    			foodSpecs: 'one',//食品规格
    			dialogFormVisible: false,//添加规格
		        specsForm: {//添加规格里的食品规格
		          	specs: '默认',
		          	packing_fee: 0,
		          	price: 20,
		        },
    	...
    	methods: {
		    ...
		    addCategoryFun(){//显示添加食品种类
		    	this.showAddCategory = !this.showAddCategory;
		    },
		    submitcategoryForm(categoryForm) {//添加食品种类
		    ...
		    addFood(foodForm){//添加食品
		    	this.$refs[foodForm].validate(async (valid) => {//表单验证
					if (valid) {
						const params = {
							...this.foodForm,//把集合转换成参数列表
