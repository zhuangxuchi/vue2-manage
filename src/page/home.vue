...
<script>
    ...
    export default {
    	...
    	methods: {
    		async initData(){//获取数据统计
    			const today = dtime().format('YYYY-MM-DD')
    			Promise.all([userCount(today), orderCount(today), adminDayCount(today), getUserCount(), getOrderCount(), adminCount()])
				//如果参数中promise有一个失败，此实例回调失败，失败原因的是第一个失败 promise 的结果。
				.then(res => {
    		...
    		async getSevenData(){//获取近7天的数据
    			...
    			const promiseArr = [...apiArr[0], ...apiArr[1], ...apiArr[2]]//连接3组数据
