...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async querySearchAsync(queryString, cb) {//获取店铺分类的输入建议
                if (queryString) {
                    try{
            ...
            addressSelect(vale){//	点击选中建议项时触发
                const {address, latitude, longitude} = vale;
                this.address = {address, latitude, longitude};
            },
            handleServiceAvatarScucess(res, file) {//上传成功时的钩子
                if (res.status == 1) {
            ...
            async updateShop(){//更新店铺信息
                this.dialogFormVisible = false;
                try{
                    Object.assign(this.selectTable, this.address);//把this.address里面this.selectTable所没有的复制到this.selectTable里
