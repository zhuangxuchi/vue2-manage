...
<script>
    ...
    export default {
        ...
        created(){
        	this.restaurant_id = this.$route.query.restaurant_id;//接收参数or获取参数
            this.initData();
        },
        mounted(){
        },
        methods: {
            async initData(){//获取订单数量和信息
                try{
            ...
            async expand(row, status){
            	if (status) {
                    ...
                    //^^把店铺名称,店铺地址,收货地址,用户名和row连接,把row里指定位置的内容替换成连接的结果
                    this.$nextTick(() => {//等其他数据更新完执行
