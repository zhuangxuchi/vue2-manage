    ...
    export default {
        data(){
            return {
                ...
                specsFormrules: {
		        	specs: [
						{ required: true, message: '请输入规格', trigger: 'blur' },//失去焦点时触发
					],
		        },
        ...
        methods: {
            ...
            tableRowClassName(row, index) {//规格表行样式
		        if (index%2 === 1) {
		        	return 'info-row';
		        } else if (index%2 === 0) {
		        	return 'positive-row';
		    ...
		    addspecs(){//添加规格
				...
				this.specsForm.specs = '默认';
				this.specsForm.packing_fee = 0;
				this.specsForm.price = 20;
				this.specsFormVisible = false;
			},
			deleteSpecs(index){//删除规格
            ...
            async getSelectItemData(row, type){
                ...
                //^^把餐馆名称,餐馆地址,食品分类和row连接,结果赋值给this.selectTable
                this.selectMenu = {label: category.name, value: row.category_id}
                this.tableData.splice(row.index, 1, {...this.selectTable});
                this.$nextTick(() => {//等待其他其他数据更新完再更新
            ...
            handleSelect(index){//变更食品分类
            	this.selectIndex = index;
            	this.selectMenu = this.menuOptions[index];
            },
            async handleDelete(index, row) {//删除食品
                try{
            ...
            async updateFood(){//更新食品信息
                this.dialogFormVisible = false;
                try{
                	const subData = {new_category_id: this.selectMenu.value, specs: this.specs};
                	const postData = {...this.selectTable, ...subData};//把subData和this.selectTable连接，结果赋值给postData
